...
export function AccountExecutionPanel() {
  ...
  return (
    <section
      className="ds-page-section scroll-mt-24 overflow-hidden"
      aria-label="Execution runbook"
    >
      <div className="border-b border-surface-border/45 bg-surface/30 px-4 py-4 sm:px-5 sm:py-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-lg border border-surface-border/50 bg-surface-muted/30 shadow-[0_1px_0_rgb(255_255_255/0.04)_inset]">
              <Workflow className="h-4 w-4 text-accent" strokeWidth={1.8} />
            </div>
            <div className="min-w-0">
              <p className="text-label text-text-faint/90">Execution</p>
              <h2 className="mt-1 text-[15px] font-semibold tracking-tight text-text-primary">
                Runbook
              </h2>
              <p className="mt-1 max-w-xl text-[11px] leading-relaxed text-text-muted">
                Brief → discovery → POV → expansion. Pick a step, run, copy into CRM or email.
              </p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full border border-surface-border/50 bg-surface-muted/25 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.08em] text-text-faint shadow-[0_1px_0_rgb(255_255_255/0.03)_inset]">
              {account.name}
            </span>
            <span className="rounded-full border border-surface-border/45 px-2.5 py-1 text-[10px] font-semibold tabular-nums text-text-muted">
        ...
        {!hasApiKey && (
          <p className="mt-4 rounded-lg border border-accent/25 bg-accent/[0.07] px-3 py-2 text-[11px] font-medium text-accent/95 shadow-[0_1px_0_rgb(255_255_255/0.04)_inset]">
                    ...
                    {phaseActions.map((a) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "ds-focus-ring flex w-full items-start justify-between gap-2 rounded-lg border px-2.5 py-2 text-left text-[11px] font-medium transition-all duration-200 ease-out-expo",
                            selected
                              ? "border-accent/35 bg-accent/[0.08] text-text-primary shadow-[inset_0_0_0_1px_rgb(41_181_232/0.12)]"
                              : "border-surface-border/40 bg-surface-muted/15 text-text-secondary hover:border-accent/20 hover:bg-surface-muted/25",
        ...
        <div className="flex min-h-[300px] flex-col border-t border-surface-border/35 lg:border-t-0">
          {!viewing && (
            <div className="flex flex-1 flex-col justify-center px-4 py-10 sm:px-6">
              <div className="mx-auto w-full max-w-md rounded-2xl border border-dashed border-surface-border/55 bg-surface-muted/[0.14] px-6 py-10 text-center shadow-[inset_0_1px_0_rgb(255_255_255/0.04)]">
                <div className="mx-auto mb-4 flex h-11 w-11 items-center justify-center rounded-xl border border-surface-border/45 bg-surface-elevated/40">
                  <Workflow className="h-5 w-5 text-accent/80" strokeWidth={1.8} />
                </div>
                <p className="text-[13px] font-semibold tracking-tight text-text-primary">No step selected</p>
                    ...
                    {tabTargets.map((t) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "ds-focus-ring shrink-0 rounded-md border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.08em] transition-all duration-200",
                            active
                              ? "border-accent/35 bg-accent/[0.11] text-accent shadow-[0_1px_0_rgb(255_255_255/0.04)_inset] ring-1 ring-accent/15"
                              : "border-transparent bg-surface-muted/25 text-text-muted hover:border-surface-border/50 hover:bg-surface-muted/35",
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5">
                    <button
                      ...
                      className={cn(
                        "ds-focus-ring inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-[11px] font-semibold transition-all duration-200",
                        activeOutput && !loadingThis
                          ? "border-surface-border/50 bg-surface-muted/30 text-text-secondary shadow-[0_1px_0_rgb(255_255_255/0.03)_inset] hover:border-accent/30 hover:text-text-primary active:scale-[0.99]"
                    ...
                    <button
                      ...
                      className={cn(
                        "ds-focus-ring inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-[11px] font-semibold transition-all duration-200",
                        hasApiKey && !isLoading
                          ? "border-accent/30 bg-accent/[0.11] text-accent shadow-[0_1px_0_rgb(255_255_255/0.05)_inset] hover:bg-accent/[0.16] active:scale-[0.99]"
              ...
              <div className="max-h-[min(440px,56vh)] flex-1 overflow-y-auto px-3 py-3 sm:px-4 sm:py-4">
                ...
                {loadingThis && !activeOutput && (
                  <div className="space-y-3 pt-1" aria-busy>
                    <div className="space-y-2.5 rounded-xl border border-surface-border/40 bg-surface-muted/12 p-4 shadow-[inset_0_1px_0_rgb(255_255_255/0.03)]">
                      <div className="h-2 w-1/3 animate-pulse rounded bg-surface-muted/55" />
                      <div className="h-2 w-full animate-pulse rounded bg-surface-muted/45" />
                      <div className="h-2 w-5/6 animate-pulse rounded bg-surface-muted/40" />
                    </div>
                    <div className="space-y-2.5 rounded-xl border border-surface-border/40 bg-surface-muted/12 p-4 shadow-[inset_0_1px_0_rgb(255_255_255/0.03)]">
                      <div className="h-2 w-2/5 animate-pulse rounded bg-surface-muted/50" />
                      <div className="h-2 w-full animate-pulse rounded bg-surface-muted/38" />
                      <div className="h-2 w-4/5 animate-pulse rounded bg-surface-muted/35" />
                      <div className="h-2 w-3/5 animate-pulse rounded bg-surface-muted/32" />
                    </div>
                    <p className="flex items-center gap-2 text-[11px] font-medium text-text-faint">
                      <Loader2 className="h-3.5 w-3.5 animate-spin text-accent/70" aria-hidden />
                      Generating output…
                    </p>
                  </div>
                )}
